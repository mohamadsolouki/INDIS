...
func TestBiometricStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
	t.Parallel()
	...
	schema := fmt.Sprintf("biometric_startup_it_%d", time.Now().UnixNano())
	...
	var biometricsTable string
	if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('biometrics')::text, '')").Scan(&biometricsTable); err != nil {
		t.Fatalf("check biometrics table: %v", err)
	}
	if biometricsTable == "" {
		t.Fatal("expected biometrics table to exist after startup migrations")
	}
