...
func TestCredentialStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
	t.Parallel()
	...
	schema := fmt.Sprintf("credential_startup_it_%d", time.Now().UnixNano())
	...
	var credentialsTable string
	if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('credentials')::text, '')").Scan(&credentialsTable); err != nil {
		t.Fatalf("check credentials table: %v", err)
	}
	if credentialsTable == "" {
		t.Fatal("expected credentials table to exist after startup migrations")
	}
