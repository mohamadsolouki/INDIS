...
func TestElectoralStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
	t.Parallel()
	...
	schema := fmt.Sprintf("electoral_startup_it_%d", time.Now().UnixNano())
	...
	var electionsTable string
	if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('elections')::text, '')").Scan(&electionsTable); err != nil {
		t.Fatalf("check elections table: %v", err)
	}
	if electionsTable == "" {
		t.Fatal("expected elections table to exist after startup migrations")
	}
