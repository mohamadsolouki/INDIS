...
func TestAuditStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
t.Parallel()
...
schema := fmt.Sprintf("audit_startup_it_%d", time.Now().UnixNano())
...
if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('audit_logs')::text, '')").Scan(&auditLogsTable); err != nil {
t.Fatalf("check audit_logs table: %v", err)
}
if auditLogsTable == "" {
t.Fatal("expected audit_logs table to exist after startup migrations")
}
