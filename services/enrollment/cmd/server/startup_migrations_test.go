...
func TestEnrollmentStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
	t.Parallel()
	...
	schema := fmt.Sprintf("enrollment_startup_it_%d", time.Now().UnixNano())
	...
	var enrollmentsTable string
	if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('enrollments')::text, '')").Scan(&enrollmentsTable); err != nil {
		t.Fatalf("check enrollments table: %v", err)
	}
	if enrollmentsTable == "" {
		t.Fatal("expected enrollments table to exist after startup migrations")
	}
