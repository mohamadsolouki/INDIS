...
func TestNotificationStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
t.Parallel()
...
schema := fmt.Sprintf("notification_startup_it_%d", time.Now().UnixNano())
...
var notificationsTable string
if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('notifications')::text, '')").Scan(&notificationsTable); err != nil {
t.Fatalf("check notifications table: %v", err)
}
if notificationsTable == "" {
t.Fatal("expected notifications table to exist after startup migrations")
}
