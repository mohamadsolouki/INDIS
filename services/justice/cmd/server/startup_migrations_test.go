...
func TestJusticeStartupAppliesMigrationsOnCleanSchema(t *testing.T) {
	t.Parallel()
	...
	schema := fmt.Sprintf("justice_startup_it_%d", time.Now().UnixNano())
	...
	var testimoniesTable string
	if err := pool.QueryRow(ctx, "SELECT COALESCE(to_regclass('testimonies')::text, '')").Scan(&testimoniesTable); err != nil {
		t.Fatalf("check testimonies table: %v", err)
	}
	if testimoniesTable == "" {
		t.Fatal("expected testimonies table to exist after startup migrations")
	}
